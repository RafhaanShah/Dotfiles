...
_prompt_git_changed() {
    ...
    local gs; gs="$(git status --porcelain --branch --ignore-submodules)"
...
_prompt_git() {
    ...
    local gitstatus; gitstatus="$(git status --porcelain --branch --ignore-submodules)"
    ...
    local upstream; upstream="$(git rev-list --left-right --count origin/"${branch}"...HEAD 2>/dev/null)"
    
    # check for divergences from upstream (origin/branch)
    if [ -n "${upstream}" ]; then
        local behind; behind="$(cut -f1 <<< "${upstream}")"
        local ahead; ahead="$(cut -f2 <<< "${upstream}")"
        local upstring
...
_prompt_git_old() {
    ...
    local fcount; fcount="$(git status --porcelain --ignore-submodules | wc -l | tr -d ' ')"
    if [ "${fcount}" -gt 0 ]; then
        s+="${fcount}"
    fi
	# check for conflicts
